...
export default function Home(){
 ...
 const [page,setPage]=useState<"cover"|"intro"|"solve"|"teach"|"end">("cover"),[question,setQuestion]=useState<Q>(1),[step,setStep]=useState(-1),[activeLine,setActiveLine]=useState<number|null>(null);
 ...
 const speakerRange=()=>{if(page==="solve"){playAll();return}const [s,e]=ranges[question];playRange(s,e+.5)};
 ...
 const next=()=>{if(page==="cover"){setPage("intro");return}if(page==="intro"){setPage("solve");return}if(page==="solve"){setPage("teach");setStep(-1);return}if(page==="end")return;if(step<finalStep(question)){setStep(step+1);return}if(question<10){setQuestion((question+1)as Q);setStep(-1);setActiveLine(null);return}setPage("end")};
 const prev=()=>{if(page==="intro"){setPage("cover");return}if(page==="solve"){setPage("intro");return}if(page==="end"){setPage("teach");setQuestion(10);setStep(finalStep(10));return}if(page==="teach"&&step>=0){setStep(step-1);return}if(page==="teach"&&question>1){const q=(question-1)as Q;setQuestion(q);setStep(finalStep(q));setActiveLine(null);return}if(page==="teach")setPage("solve")};
 ...
 const currentKnowledge=knowledge[question],nextLabel=page==="solve"?"进入讲解":question===10&&step===finalStep(10)?"返回首页":step===finalStep(question)?`讲解 Q${question+1}`:"继续";
 return <main className="stage"><audio ref={audioRef} src={asset("q1-q10.mp3")}/>{page==="cover"?<section className="cover-slide" role="button" tabIndex={0} aria-label="点击进入题目" onClick={()=>{pauseAudio();next()}} onKeyDown={e=>{if(e.key==="Enter"){pauseAudio();next()}}}><img src={asset("C20T1P1-demo.svg")} alt="" draggable={false}/></section>:page==="intro"?<section className="image-slide intro-slide" role="button" tabIndex={0} aria-label="点击进入答题页" onClick={()=>{pauseAudio();next()}} onKeyDown={e=>{if(e.key==="Enter"){pauseAudio();next()}}}><img src={asset("intro-question.png")} alt="Questions 1–10 完整题目" draggable={false}/></section>:page==="end"?<section className="image-slide final-slide"><img src={asset("final-answers.png")} alt="Questions 1–10 完整答案" draggable={false}/></section>:<section className="slide-shell" onClick={clearEmphasis}>
  <header className="topbar"><span className="editable-title" contentEditable suppressContentEditableWarning>C20-T1-P1 表格填空题</span></header><div className={`workspace ${page}`}><section className="question-panel"><div className="instruction"><b>Questions {page==="solve"?"1–2":`${groupStart(question)}–${groupEnd(question)}`}</b><span>Write <strong>ONE WORD AND/OR A NUMBER</strong> for each answer.</span></div><div ref={questionRef} className="question-stage" onClick={e=>e.stopPropagation()} onPointerDown={page==="solve"?down:undefined} onPointerMove={page==="solve"?move:undefined} onPointerUp={page==="solve"?up:undefined} onPointerCancel={page==="solve"?up:undefined}><table onClickCapture={dismiss}><thead><tr><th>Name of<br/>restaurant</th><th>Location</th><th>Reason for<br/>recommendation</th><th>Other comments</th></tr></thead><tbody>{page==="solve"?row12:table}</tbody></table>{page==="solve"&&<svg className="ink-layer"><defs><marker id="arrow" markerWidth="9" markerHeight="9" refX="7" refY="3.5" orient="auto"><path d="M0,0 L0,7 L8,3.5 z"/></marker></defs>{[...strokes,current].filter(x=>x.length>1).map((s,i)=><path key={i} d={path(s)} markerEnd="url(#arrow)"/>)}</svg>}</div></section>
  {page==="teach"&&<section className="teaching-panel"><div className="transcript-head"><span>TRANSCRIPT</span><button className="speaker" onClick={e=>{e.stopPropagation();speakerRange()}} aria-label={`播放第${question}题相关音频`}>🔊</button></div><div className="transcript">{transcriptFor(question).map((l,i)=><p key={i} className={activeLine===i?"active":""} onClick={e=>playLine(e,i,l)}><b>{l.speaker}:</b><span>{l.sentences.map((s,j)=><span key={j} className="sentence" onDoubleClick={e=>{e.stopPropagation();playSentence(i,s)}}>{j>0&&" "}{s.text}</span>)}</span></p>)}</div><div className="explain-card">{step===0&&<p><em>答案</em><strong className="answer">{answers[question]}</strong></p>}{step===1&&currentKnowledge&&<div className="q1-knowledge">{currentKnowledge.synonym&&<p><em>同义替换</em><span>{currentKnowledge.synonym}</span></p>}{currentKnowledge.vocabulary&&<p><em>重点词汇</em><span>{currentKnowledge.vocabulary}</span></p>}</div>}</div></section>}</div>
  <footer><button className="speaker footer-speaker" onClick={e=>{e.stopPropagation();speakerRange()}} aria-label={page==="solve"?"播放全部音频":"播放对应音频片段"}>🔊</button><div className="nav"><button onClick={prev}>←</button><button className="next" onClick={next}>{nextLabel} →</button></div></footer></section>}</main>
}
